...
exports.superadminanduponly= async (req,res,next)=>{
    try {
        ...
        if(response.role_id!=='super_admin'||response.role_id!=='Owner') return res.status(405).json({msg:'you are not allowed to use this',err:""})
            next();
    } catch (error) {
